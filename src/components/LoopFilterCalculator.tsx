...
const REF_FREQS: RefFrequency[] = [1000, 5000, 9000, 10000, 25000, 50000, 100000]
...
export default function LoopFilterCalculator() {
  ...
  const result = useMemo(() => calculateLoopFilter(input), [
    input.fRF, input.fIF, input.fref, input.Icp, input.Kvco, input.fc, input.phiM,
  ])
  ...
  const plotRefs = useMemo(() => {
    ...
    return [
      ...
      { logF: Math.log10(result.value.fz), label: 'fz', color: 'rgba(0,136,255,0.7)' },
      { logF: Math.log10(result.value.fp), label: 'fp', color: 'rgba(0,136,255,0.7)' },
    ]
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-6">
      ...
      <div className="flex flex-col gap-4">
        {/* VCO Configuration */}
        <Section title="VCO Configuration" badge="RF">
          <Field id={`${id}-frf`} label="RF frequency" unit="MHz" hint="87.5–108 MHz">
            <input
              id={`${id}-frf`}
              className="field"
              type="number"
          ...
          <Field id={`${id}-fif`} label="IF frequency" unit="MHz" hint="std: 10.7">
            <input
              id={`${id}-fif`}
              className="field"
              type="number"
          ...
          <Field id={`${id}-fref`} label="Reference freq" unit="kHz" hint="chip register">
            <select
              id={`${id}-fref`}
        ...
        {/* PLL Parameters */}
        <Section title="PLL Parameters" badge="CHIP">
          <Field id={`${id}-icp`} label="Charge pump Icp" unit="mA" hint="measure on PD1/PD2">
            <input
              id={`${id}-icp`}
              className="field"
              type="number"
          ...
          <Field id={`${id}-kvco`} label="VCO gain Kvco" unit="MHz/V" hint="measured">
            <input
              id={`${id}-kvco`}
              className="field"
              type="number"
        ...
        {/* Loop Design */}
        <Section title="Loop Design" badge="DESIGN">
          <Field id={`${id}-fc`} label="Loop bandwidth fc" unit="kHz" hint="suggest: fref/10">
            <input
              id={`${id}-fc`}
              className="field"
              type="number"
          ...
          <Field id={`${id}-phim`} label="Phase margin φm" unit="°" hint="30–70°, typ 45°">
            <input
              id={`${id}-phim`}
              className="field"
              type="number"
        ...
        {result.ok && (
          <>
            ...
            <div className="bg-bg-panel border border-accent-border/30 rounded-sm p-4">
              <div className="font-mono text-xs text-text-dim mb-2 tracking-wider">TOPOLOGY</div>
...
{`PD1/PD2 ──┬──── R ────┬──── Vtune(VCO)
          ...
          GND         GND`}
            ...
            {/* Results */}
            <ResultsPanel
              title="Computed Component Values"
              rows={rows}
              warning={
                result.value.stabilityWarning
                  ? `fc = ${(result.value.fc_actual / 1000).toFixed(2)} kHz > fref/10 = ${(fref / 10000).toFixed(1)} kHz — reference spur injection risk`
                  : undefined
              }
            />
            ...
            {plotData.length > 0 && (
              <FrequencyPlot
                data={plotData}
                refs={plotRefs}
                title="Open-Loop Bode Plot  |G(jω)|"
                showPhase={true}
              />
            )}
          </>
    ...
    <div className="grid grid-cols-[1fr_auto] gap-x-3 gap-y-1 items-start">
      <div>
        ...
        >
          {label}
          {hint && (
            <span className="ml-2 font-mono text-xs text-text-dim">[{hint}]</span>
          )}
        </label>
        {children}
      </div>
